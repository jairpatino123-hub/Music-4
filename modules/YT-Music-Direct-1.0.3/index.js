...
(function () {
  ...
  var SEARCH_CACHE_TTL_MS = 120000;
  var searchCache = Object.create(null);
  var searchInflight = Object.create(null);
  ...
  async function searchResults(query, page) {
    ...
    var cacheKey = term.toLowerCase().replace(/\s+/g, ' ');
    var cached = searchCache[cacheKey];
    if (cached && (Date.now() - cached.at) < SEARCH_CACHE_TTL_MS) return ok(cached.items);
    if (searchInflight[cacheKey]) return searchInflight[cacheKey];

    var warmKeys = Object.keys(searchCache);
    for (var wk = 0; wk < warmKeys.length; wk++) {
      var warmKey = warmKeys[wk];
      var warm = searchCache[warmKey];
      if (!warm || (Date.now() - warm.at) >= SEARCH_CACHE_TTL_MS) continue;
      if (warmKey.length >= cacheKey.length || cacheKey.indexOf(warmKey) !== 0) continue;
      var warmItems = warm.items.filter(function (it) {
        var hay = ((it.title || '') + ' ' + (it.artist || '') + ' ' + (it.album || '')).toLowerCase();
        var tokens = cacheKey.split(' ');
        for (var ti = 0; ti < tokens.length; ti++) if (hay.indexOf(tokens[ti]) === -1) return false;
        return true;
      });
      if (warmItems.length) return ok(warmItems);
    }
    ...
    var request = (async function () {
      ...
      delete searchInflight[cacheKey];
      return ok(items);
      } catch (err) {
        delete searchInflight[cacheKey];
    ...
    searchInflight[cacheKey] = request;
    return request;
  }
